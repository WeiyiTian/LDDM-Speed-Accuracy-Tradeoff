[R1_ave, R2_ave] = ave_choice(.4);
h = figure; hold on;
plot(R1_ave,'k-','LineWidth',3);
plot(R2_ave,'k--','LineWidth',3);
legend({'R1','R2'})
xlabel('t/msecs');
ylabel('Firing rates/Hz');

function [R1_ave, R2_ave] = ave_choice(r)
    % parameters
    dt = .001;
    tau = .1;
    noise_tau = .002;
...
%     choice = 0;
    ...
    for rep = 2:(1/dt)
        ...
        for ti = 2:(dur/dt)
            ...
            R1_ave(ti) = R1_ave(ti) + R1(ti);
            R2_ave(ti) = R2_ave(ti) + R2(ti);
