function [rt, choice, argmaxR] = LDDM_GPU_ABMtrx(Vprior, Vinput, w, a, b,...
...
sizeA = size(a);
if ~isequal(sizeVinput,sizeA)
    error('dimensions of Vinput matrix does not match alpha matrix');
end
...
if isequal(size(offset_of_stimuli), sizeA)
...
for ti = -pretask_steps:max(posttask_steps(:))
    ...
    I1 = I1 + (-I1 + beta.*R1.*Continue.*BetasUp)/Tau3*dtArray + InoiseI1;
    I2 = I2 + (-I2 + beta.*R2.*Continue.*BetasUp)/Tau3*dtArray + InoiseI2;
    R1 = R1 + (-R1 + (V1Array + V1prArray*(ti<0) + alpha.*R1).*Continue./(1+G1old))/Tau1*dtArray + InoiseR1;
    R2 = R2 + (-R2 + (V2Array + V2prArray*(ti<0) + alpha.*R2).*Continue./(1+G2old))/Tau1*dtArray + InoiseR2;
    % update noise
    InoiseR1 = InoiseR1 + (-InoiseR1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
    InoiseR2 = InoiseR2 + (-InoiseR2 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
%     InoiseG1 = InoiseG1 + (-InoiseG1 + gpuArray.randn(sizeComput)*sqrt(dtArray)*sgmArray)/tauN*dtArray;
