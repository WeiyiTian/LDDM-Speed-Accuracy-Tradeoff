...
%   a_acc,  b_acc, a_speed, b_speed, noise, noiseInput, scale,  Tau
LB = [0     0.5      0      0.5      .1      0          .1*256  [.01,.01,.01]];
UB = [70    2.5	    70      2.5      100     50         20*256  [1,1,1]];
PLB = [0   .9	    15      .9       5       5          1*256   [.02 .02 .02]];
PUB = [50   1.7	    60      1.7      40      15         8*256   [.2 .2 .2]];
...
parfor i = 1:myCluster.NumWorkers*4
    ...
    % Randomize initial starting point inside plausible box
    x0 = rand(1,numel(LB)) .* (PUB - PLB) + PLB;
    dlmwrite(fullfile(out_dir,'Ev10_x0List.txt'),[sortNum, i, t, x0],'delimiter','\t','precision','%.6f','-append');
    % fit
    ...
    dlmwrite(fullfile(out_dir,'Ev10_RsltList.txt'),[sortNum, i, t, xest fval],'delimiter','\t','precision','%.6f','-append');
...
t = datenum(clock)*10^10 - floor(datenum(clock)*100)*10^8 + sortNum*10^7 + i*10^5;
save(fullfile(out_dir,sprintf('Ev10_CollectRslts%i.mat',t)),'Collect');
