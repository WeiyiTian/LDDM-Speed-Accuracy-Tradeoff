...
UB = [70    3	    70      3       5           20*256  [1,1,1]];
PLB = [15   .9	    15      .9      2          1*256   [.01 .01 .01]];
PUB = [60   1.7	    60      1.7     3          8*256   [.2 .2 .2]];
...
parfor i = 1:myCluster.NumWorkers*4
    ...
    % Randomize initial starting point inside plausible box
    x0 = rand(1,numel(LB)) .* (PUB - PLB) + PLB;
    dlmwrite(fullfile(out_dir,'Dv7_x0List.txt'),[sortNum, i, t, x0],'delimiter','\t','precision','%.6f','-append');
    % fit
    ...
    dlmwrite(fullfile(out_dir,'Dv7_RsltList.txt'),[sortNum, i, t, xest fval],'delimiter','\t','precision','%.6f','-append');
...
t = datenum(clock)*10^10 - floor(datenum(clock)*100)*10^8 + sortNum*10^7 + i*10^5;
save(fullfile(out_dir,sprintf('Dv7_CollectRslts%i.mat',t)),'Collect');
