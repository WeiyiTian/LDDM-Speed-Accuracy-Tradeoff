...
function LL = test_QMLE(simulate_data, empirical_data)
...
for vi = 1: length(speed_coh)
    ...
    speed_total(vi) = length(simulate_speed_rt(:, vi));
    ...
    speed_f(:, :, vi) = log(speed_estimate_qnum(:, :, vi) / speed_total(vi));
    speed_f(speed_f(:, 1, vi)==-Inf, 1, vi) = log(.1/speed_total(vi));
    speed_f(speed_f(:, 2, vi)==-Inf, 2, vi) = log(.1/speed_total(vi));
end
